...
const EmployeeTableRow = ({ employee }) => (
  <tr className="hover:bg-gray-50">
    ...
    <td className="px-6 py-4 text-sm text-gray-500">
      <button className="text-indigo-600 hover:text-indigo-900 mr-3 p-1" title="Edit">
        <FiEdit />
      </button>
...
const UserTestsTable = () => {
  ...
  const handleViewResults = (test) => {
    navigate('/results', {
      state: { ...test, isFromHistory: true, email: test.email },
    });
  };
...
const HRDashboard = () => {
  ...
  const baseEmployees = useMemo(
    ...
    { id: 51, name: 'Wyatt Sethi', position: 'Customer Success Manager', department: 'Support', status: 'active', joinedDate: '2024-12-08' },
    { id: 52, name: 'Xena Parekh', position: 'Graphic Designer', department: 'Design', status: 'on leave', joinedDate: '2024-04-20' },
    ...
    { id: 61, name: 'Gopal Shukla', position: 'UX Designer', department: 'Design', status: 'active', joinedDate: '2024-08-28' },
  ...
  const stats = useMemo(
    () => [
      ...
      {
        title: 'On Leave',
        value: baseEmployees.filter((e) => e.status === 'on leave').length,
        change: '+3',
  ...
  const departmentDistribution = useMemo(() => {
    const counts = baseEmployees.reduce((acc, emp) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      <div
        ...
        } bg-gradient-to-b from-blue-400 to-white text-white transition-all duration-300 flex flex-col`}
      >
        <div className="p-4 flex items-center justify-between border-b border-indigo-700 border-opacity-50">
          {sidebarOpen ? (
            <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-purple-200">
              HR<span className="text-white">Pro</span>
            </h1>
          ) : (
          ...
          {[
            ...
            <Link
              ...
              className={`flex items-center w-full p-3 ${
                ...
                activeTab === item.id ? 'text-black bg-blue-400 hover:bg-opacity-50' : 'text-black hover:bg-blue-400 hover:bg-opacity-50'
              }`}
        ...
        <div className={`p-3 ${sidebarOpen ? 'border-t border-indigo-700 border-opacity-50' : ''}`}>
          <button
            ...
            className={`flex items-center w-full p-3 ${
              ...
              activeTab === 'settings' ? 'text-black' : 'text-black hover:bg-indigo-700 hover:bg-opacity-50'
            }`}
        ...
        <header className="bg-white shadow-sm z-10">
          <div className="flex items-center justify-between p-4">
            <h2 className="text-xl font-semibold text-gray-800 capitalize">
              {activeTab === 'dashboard' ? 'HR Dashboard' : activeTab.replace('-', ' ')}
            </h2>
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {stats.map((stat, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-indigo-500 hover:shadow-indigo-100 transition-shadow"
                  >
                  ...
                  <div className="space-y-4 max-h-72 overflow-y-auto">
                    {upcomingEvents.length > 0 ? (
                      upcomingEvents.map((event) => (
                        ...
                        >
                          <div
                            className={`p-2.5 rounded-lg mr-4 text-xl ${
              ...
              <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="flex justify-between items-center p-6 border-b border-gray-200">
                  <h3 className="text-lg font-semibold text-gray-800">
                    Recent Employees Activity ({recentEmployeesForDashboard.length})
                  </h3>
                  <button
                ...
                {recentEmployeesForDashboard.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {recentEmployeesForDashboard.map((employee) => (
                  ...
                  <p className="text-center text-gray-500 py-8">
                    {searchTerm ? `No employees found matching "${searchTerm}".` : 'No recent employees to display.'}
                  </p>
                )}
              </div>
            </>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              {filteredEmployees.length > 0 ? (
                ...
                <p className="text-center text-gray-500 py-10">
                  {searchTerm ? `No employees found matching "${searchTerm}".` : 'No employees in the system.'}
                </p>
              )}
            </div>
          )}
          {activeTab === 'results' && <UserTestsTable />}
          {['recruitment', 'interviewresults', 'joining-process', 'settings'].includes(activeTab) && (
            <div className="bg-white rounded-xl shadow-lg p-8">
              ...
              <div className="text-gray-600 space-y-4">
                <p>
                  The <strong className="font-medium">{activeTab.replace('-', ' ')}</strong> section is under
                  development.
                </p>
                <p>
                  We're working on a comprehensive interface for managing {activeTab.replace('-', ' ')}. Check back soon!
                </p>
