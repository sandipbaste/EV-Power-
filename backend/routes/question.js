...
const seedInitialQuestions = async () => {
  try {
    ...
    { text: "Which renewable energy source is commonly used to charge electric vehicles?", options: ["Solar energy", "Wind energy", "Hydropower", "Geothermal"], answer: "Solar energy" },
      { text: "What component in an EV ensures safety during battery operation?", options: ["Cooling system", "Battery Management System (BMS)", "Electric motor", "Transmission"], answer: "Battery Management System (BMS)" },
  { text: "Which government policy can encourage EV adoption?", options: ["Fuel tax increase", "Subsidies for EV purchases", "Banning solar panels", "Raising toll taxes"], answer: "Subsidies for EV purchases" },
  { text: "What is the function of an EV’s thermal management system?", options: ["Improve sound quality", "Enhance acceleration", "Regulate battery and motor temperature", "Increase top speed"], answer: "Regulate battery and motor temperature" },
  { text: "What type of maintenance do EVs generally require less of compared to ICE vehicles?", options: ["Battery replacement", "Brake pad replacement", "Oil changes", "Tire rotation"], answer: "Oil changes" },
  { text: "Which of the following is NOT a type of electric vehicle?", options: ["Battery Electric Vehicle (BEV)", "Plug-in Hybrid Electric Vehicle (PHEV)", "Internal Combustion Engine Vehicle (ICEV)", "Hybrid Electric Vehicle (HEV)"], answer: "Internal Combustion Engine Vehicle (ICEV)" },
  { text: "What determines the charging speed of an EV?", options: ["Size of tires", "Type of charger and battery capacity", "Color of vehicle", "Length of trip"], answer: "Type of charger and battery capacity" },
  { text: "Which country has the highest EV adoption rate as of recent years?", options: ["USA", "China", "Norway", "Germany"], answer: "Norway" },
  { text: "How does an EV contribute to quieter cities?", options: ["They use special soundproof tires", "They generate less engine noise", "They are driven only during the day", "They run on solar power only"], answer: "They generate less engine noise" },
  { text: "What safety concern is associated with EV battery packs?", options: ["High flammability", "Overheating and thermal runaway", "Water leakage", "Brake failure"], answer: "Overheating and thermal runaway" },
  { text: "What is one way EVs support smart grid systems?", options: ["By using more electricity", "By storing excess renewable energy", "By using gasoline as backup", "By powering other vehicles"], answer: "By storing excess renewable energy" }
  ]
    ...
    const result = await Question.insertMany(initialQuestions);
...
router.post('/bulk', async (req, res) => {
  try {
    ...
    const existingQuestions = await Question.find({
      text: { $in: req.body.map(q => q.text) }
    });

    const existingTexts = new Set(existingQuestions.map(q => q.text));
    const newQuestions = req.body.filter(q => !existingTexts.has(q.text));
